...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	GetCursorPos(&p);//获得屏幕坐标
	GetWindowRect(hWnd, &m_rRect);//获得窗口左上角位置
	tagx = p.x - m_rRect.left - 3;
	tagy = p.y - m_rRect.top - 29;
	
	sprintf_s(hx, "%d", tagx);
	jx =hx;
	sprintf_s(hy, "%d", tagy);
	jy =hy;
	if (tagx >200.0f && tagy >30.0f && tagx < 600 && tagy < 100)
	{
		//MessageBox(NULL, jy, jx, MB_ICONERROR | MB_OK);
		pInstance->m_InputMgr->l_button_down = true;
	}
	...
	//pInstance->m_InputMgr->getMouseXPos()=p.x;
	}
		break;
	case WM_KEYDOWN:
	{
